...
| [N](#cycle-n--v0210--audit--two-ai-audit-authrbac--backup-secrets--relaysync--recursor--abuse-engine) | v0.21.0 → audit | 2026-06-20 | [AI-INTERNAL] Claude Opus 4.8 (fresh-context agent) × [AI-ADVERSARIAL] Gemini 2.5 Pro | 2 HIGH fixed (SEC-N1 backup-secret-leak, SEC-N2 DNS zone-scope); 5 more fixed; 2 Gemini CRITICALs disputed at code; OPEN-N1/N2 open (enhancements). Remediation slow-path only; datapath unchanged |
...
**Remediation & datapath:** all fixes are in the API/sync/config slow path. The XDP/eBPF/AF_XDP packet code is unchanged; the internal pass re-confirmed the datapath bounds, the recursor `Bogus → SERVFAIL` enforcement, the HMAC/TOFU model and the backup/TLS path-traversal guards as carrying no finding. CI gate on the four remediation commits (`audit/2ai-hardening`): `cargo clippy --all-targets -- -D warnings` clean, `cargo test --all-targets` 312 passed / 0 failed (2 network-ignored). Pending: maintainer deploy to master + slave and release.
