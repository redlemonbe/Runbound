...
**Current version:** v0.21.0 (Cycle N two-AI audit of the auth/RBAC + backup-secrets + relay/sync + sovereign-recursor + abuse-engine surface — SEC-N1 (HIGH) `backup/export` secret-leak **fixed**, SEC-N2 (HIGH) DNS zone-scope bypass **fixed**, 5 more fixed, 2 Gemini CRITICALs disputed at the code, OPEN-N1/N2 enhancements; remediation CI green (`clippy --all-targets -D warnings` + 312 tests). The Gemini cross-pass caught the real SEC-N2 the internal pass missed; the internal pass refuted both Gemini CRITICALs at the role middleware. Prior: Cycle M two-AI audit of the DoH-serving / local-CA / DDoS-abuse-engine / audit-attribution / #208-metrics delta — SEC-M1 (HIGH) config-injection via the alert-rule writer **fixed**; SEC-M2 (LOW) accepted; SEC-M3 Gemini-disputed; XDP datapath delta = one gated ban lookup, A/B bench ±0.12% no regression. Prior: Cycle L two-model adversarial audit of the release delta — #201 DNSSEC signing, DoT/DoH/DoQ WebUI, #202 sovereign recursion. SEC-L0 (HIGH) TCP-tracker deadlock DoS fixed; 8 more fixed, 1 accepted (SEC-L12), OPEN-L1 open; 0 new CRITICAL/HIGH. Gemini 2.5 Pro cross-model pass completed (both disputed-down). Live pentest: no exploitable finding. Prior: Cycle K v0.19.3 anycast)  
...
| [M](#cycle-m--v0200--main--two-ai-audit-doh-serving--local-ca--ddos-abuse-engine--audit-attribution--208-metrics) | v0.20.0 → main | 2026-06-19 | [AI-INTERNAL] Claude Opus 4.8 × [AI-ADVERSARIAL] Gemini 2.5 Pro | 1 HIGH fixed (SEC-M1 config-injection); 1 LOW accepted (SEC-M2); 1 Gemini disputed (SEC-M3); datapath delta = gated XDP ban lookup, A/B X710 bench ±0.12% (no regression) |
| [N](#cycle-n--v0210--audit--two-ai-audit-authrbac--backup-secrets--relaysync--recursor--abuse-engine) | v0.21.0 → audit | 2026-06-20 | [AI-INTERNAL] Claude Opus 4.8 (fresh-context agent) × [AI-ADVERSARIAL] Gemini 2.5 Pro | 2 HIGH fixed (SEC-N1 backup-secret-leak, SEC-N2 DNS zone-scope); 5 more fixed; 2 Gemini CRITICALs disputed at code; OPEN-N1/N2 open (enhancements). Live pentest confirmed SEC-N1/N2 under attack and found NEW-N1 (SEC-N10, RBAC fail-closed) — fixed. 317 tests green |
...
**Cycle M (v0.20.0→main)** audited the DoH own-handler serving, the local-CA cert signer (`/api/tls/ca`), the DDoS abuse engine (verified-source gate + tarpit + kernel ban), audit actor attribution, the alert-rule API and `/metrics` (#208). **SEC-M1 (HIGH)** — alert-rule string fields (notably `notify-url`) were written to the config **unescaped**, a directive-injection vector of the SEC-J1 class (a malicious value with an embedded newline could inject a separate directive such as a command-running hook) — is **Fixed** (writer `escape_str` on every alert string field + `notify-url` control-char/length validation in `PUT /api/alerts/rules`). **SEC-M2 (LOW)** authenticated-only internal-error text in API error bodies is **Accepted** (consistent codebase pattern, admin/API-key only). Gemini's **SEC-M3** (claimed `/api/tls/self-signed` removed hostname validation → DoS) is **Disputed** — the 1..253-char/no-control validation is present and was never removed. Unlike prior cycles the XDP datapath was **not** byte-identical: it gained one `bans_active`-gated ban lookup per DNS packet, but the A/B X710 NIC-truth bench (gate vs pre-gate baseline, two rounds) measured ±0.12% — within run-to-run noise — so no fast-path regression was introduced; the BPF passed the verifier at load.

**Cycle N (v0.21.0 audit)** re-audited the REST API auth/RBAC, the backup export/import handlers, the HMAC relay/sync trust model, the #202 sovereign recursor, the DDoS abuse engine, the config writer and the XDP/eBPF datapath, two-AI. **SEC-N1 (HIGH)** — `GET /api/backup/export` dumped the config and the secret state files (master API key, relay HMAC key, WebUI CA private key) with **no admin gate**, so in multi-user mode a non-admin key could exfiltrate them (privilege escalation) — is **Fixed** (admin gate on all five backup handlers; single-user is a no-op). **SEC-N2 (HIGH)** — `add_dns_handler`/`delete_dns_handler` never enforced the per-user `zone_prefixes`, letting a non-admin Dns/Operator user write outside their zone — is **Fixed** (`may_manage_name` on add and delete). This one was surfaced by the Gemini cross-pass and missed by the internal pass. SEC-N3 (MEDIUM, ban allowlist), SEC-N4/N5 (LOW, relay body-cap + HMAC replay nonce) and SEC-N6/N7 (INFO, config escaping + webhook SSRF resolve) are **Fixed**; **OPEN-N1** (thread upstream addrs into the ban allowlist) and **OPEN-N2** (pin the SSRF-vetted IP to close the DNS-rebinding TOCTOU) are **Open** enhancements. Both Gemini CRITICALs (rotate-key, relay-forward) are **Disputed** — the role middleware (`mod.rs:546`) already blocks non-admin writes there; explicit handler gates were added anyway. All remediation is in the slow-path API/sync/config layers; the packet hot path is unchanged.

---

## Cycle N — v0.21.0 → audit — two-AI audit (auth/RBAC + backup secrets + relay/sync + recursor + abuse engine)

**Sources:** [AI-INTERNAL] Claude Opus 4.8 — fresh-context read-only audit of the security-critical surface (`src/api/`, `src/dns/server.rs` recursion, `src/dns/xdp/`, `src/sync.rs`, `src/config/writer.rs`, `src/alerts.rs`, `src/webui/`, `ebpf/dns_xdp.c`), every finding verified at the source. [AI-ADVERSARIAL] Gemini 2.5 Pro — independent cross-model pass over the same eight files (731 KB). Consistent with prior cycles Gemini over-rated: both its CRITICALs are refuted at the code (the role middleware at `mod.rs:546` already 403s non-admin writes), but the cross-pass surfaced one real HIGH the internal pass missed (SEC-N2). The internal pass documented the negative space; both passes agreed the XDP/eBPF datapath, the recursor DNSSEC-Bogus enforcement, the HMAC/TOFU model and the backup/TLS path-traversal guards carry no finding.

**Scope:** the REST API auth/RBAC (`security_middleware` + per-user `may_write`/`may_manage_name`), the backup export/import/restore handlers, the HMAC-SHA256 relay/sync trust model, the #202 sovereign recursor's DNSSEC-Bogus enforcement, the DDoS abuse/ban engine, the config writer, the webhook SSRF guard, and the XDP/eBPF datapath bounds. All remediation landed in the slow-path API/sync/config layers — the packet hot path is unchanged.

| ID | Severity | Status | Finding |
|---|---|---|---|
| SEC-N1 | HIGH | ✅ Fixed | `GET /api/backup/export` (`backup_export_handler`) had no admin gate — it base64-dumps `runbound.conf` plus the secret state files (`api.key`, `sync-key.pem`, `webui-ca-key.pem`, `webui-auth.conf`) to any authenticated key. In multi-user mode a non-admin (Read/Dns/Operator) key could exfiltrate the master API key, the relay HMAC key and the WebUI CA private key — full privilege escalation. (Single-user: no-op — the only key is admin.) Fixed: the `create_user_handler` admin gate added to the export, import, restore, create and delete backup handlers. |
| SEC-N2 | HIGH | ✅ Fixed | `add_dns_handler` / `delete_dns_handler` enforced only the path-level role (`mod.rs:546`), never the per-user zone scope, so a non-admin `Dns`/`Operator` user could create or delete any record outside their assigned `zone_prefixes`. Surfaced by the Gemini cross-pass; missed by the internal pass. Fixed: `caller.may_manage_name(name)` checked on the validated entry name (add) and on the loaded entry name before removal (delete); admin bypasses. |
| SEC-N3 | MEDIUM | ✅ Fixed (partial) | The kernel ban engine (`block_manual`/`block_bot`/`trigger`) had no protected-IP allowlist: a verified high-QPS source at a shared egress (CGNAT/corporate NAT) could be banned, blackholing DNS for every co-located user. The spoofed-victim variant is already prevented (only verified sources escalate). Fixed: loopback/unspecified ban-exemption. Configured upstream-resolver addresses are not yet threaded into the tracker → **OPEN-N1**. |
| SEC-N4 | LOW | ✅ Fixed | The slave relay/register receiver buffered the request body before enforcing the 64 KiB cap (memory amplification, bounded by the TLS + HMAC gate). Fixed: `http_body_util::Limited` caps the body (64 KiB relay / 4 KiB register) before `collect()`, matching the master-side `to_bytes` cap. |
| SEC-N5 | LOW | ✅ Fixed | The relay/sync HMAC accepted any valid signature within the ±30 s window with no replay nonce. Fixed: a bounded seen-signature cache (recorded only after the HMAC verifies, pruned to the window, 16 384 hard cap) rejects a replayed valid signature. Fails open on lock poisoning — the HMAC and the window still gate authenticity. |
| SEC-N6 | INFO | ✅ Fixed | Several config-writer string fields (`ui-*`, `firewall-tag`, `block-page-bypass-pin`, `ui-acme-*`) were written unescaped. **Not reachable across a trust boundary** — config-file only, no API setter; the historically exploitable API-settable field (split-horizon `name`) is already escaped. Fixed as defence-in-depth: `escape_str` on all of them. |
| SEC-N7 | INFO | ✅ Fixed (partial) | The alert-webhook SSRF guard rejected only literal-IP hosts, not hostnames resolving to private/metadata addresses (the URL is admin-configured). Fixed: hostnames are resolved and every resolved IP re-checked (now also IPv6 ULA `fc00::/7` / link-local `fe80::/10`), fail-closed. A resolve/connect TOCTOU (classic DNS-rebinding) remains → **OPEN-N2**. |
| SEC-N8 | — | ⛔ Disputed | Gemini rated "missing admin authz on `POST /api/rotate-key`" CRITICAL. Refuted at the code — the role middleware (`mod.rs:546`) already 403s any non-admin write on `/api/rotate-key` (it is in no non-admin `may_write` prefix). An explicit handler-level admin gate was nonetheless added (defence-in-depth). |
| SEC-N9 | LOW | ⛔ Disputed (down) | Gemini rated "any user can relay arbitrary master-signed commands to slaves" CRITICAL. Refuted for the write path — the middleware blocks non-admin writes to `/api/nodes/.../relay/*`; only a GET read-relay is reachable by a Read-role user (LOW read-through to slave state). Explicit admin gate added to `relay_forward_handler`. |
| SEC-N10 | LOW (sec) / HIGH (func) | ✅ Fixed | **NEW-N1 — found by the live pentest.** `security_middleware` (`mod.rs:546`) called `may_write` with the axum-**stripped** path (`/dns`, not `/api/dns`): the auth layer sits on the inner router that the top level mounts via `nest("/api", ...)`, and `nest` applies `StripPrefix`. `may_write` keys on `/api/...` prefixes → always false for a non-admin → every non-admin POST/DELETE fail-closed (403), so the write-RBAC for `Dns`/`Operator` roles was inert and the SEC-N2 zone-scope check was dead code. Security impact LOW (fail-closed, no escalation); functional impact HIGH. Latent since RBAC was introduced (`b7de11f8`) — invisible to both static passes. Fixed: `may_write` now keys on the `OriginalUri` un-stripped path; an HTTP-level regression test drives a non-admin role through the full nested router (the exact coverage gap). SEC-N2 zone-scope is now reachable and live. |

**OPEN-N1** (MEDIUM, enhancement) — thread the configured upstream-resolver addresses into the ban allowlist so a self-hosted resolver cannot be banned by its own traffic. **OPEN-N2** (INFO, enhancement) — pin the SSRF-vetted resolved IP into the webhook request to close the resolve/connect TOCTOU.

**Remediation & datapath:** all fixes are in the API/sync/config slow path. The XDP/eBPF/AF_XDP packet code is unchanged; the internal pass re-confirmed the datapath bounds, the recursor `Bogus → SERVFAIL` enforcement, the HMAC/TOFU model and the backup/TLS path-traversal guards as carrying no finding. CI gate on `audit/2ai-hardening`: `cargo clippy --all-targets -- -D warnings` clean, `cargo test --all-targets` 317 passed / 0 failed (2 network-ignored).

**Live pentest (Cycle N).** A disposable hardened instance (multi-user, with a non-admin `Dns` user scoped to one zone) was attacked on the build worker — the production master/slave were never contacted; the instance was torn down afterwards. **SEC-N1 was confirmed fixed under live attack:** every backup endpoint returns 403 to the non-admin key with zero secret leakage, while admin export returns a real backup (no false lockout). The negative space held: 9/9 zone-escape variants 403, SSRF to loopback / IPv4-mapped / link-local / metadata 400, routing and backup-delete path-traversal rejected, config-directive injection blocked at two layers, DNSSEC-bogus → SERVFAIL, the honeypot bans bots but exempts loopback, the brute-force brake engages without locking the legitimate admin. Gemini 2.5 Pro contributed vectors — its real ones were already mitigated and several were false positives (a non-existent `/api/dns/lookup` `server` field; case / path-normalization bypass — the router is case-sensitive and rejects `..`). The pentest surfaced **NEW-N1 (SEC-N10)** — a runtime RBAC fail-closed bug invisible to both static passes — now fixed. Pending: maintainer deploy to master + slave and release.

---

## Cycle M — v0.20.0 → main — two-AI audit (DoH serving + local-CA + DDoS abuse engine + audit attribution + #208 metrics)

**Sources:** [AI-INTERNAL] Claude Opus 4.8 — adversarial review of the full session delta (`git diff 12c79c68 -- 'src/*.rs' 'ebpf/*.c'`, 12 files), required to refute non-exploitable candidates at the source. [AI-ADVERSARIAL] Gemini 2.5 Pro — cross-model pass over the same diff. Consistent with prior cycles Gemini over-rated; one of its two findings is refuted at the code (SEC-M3) and the other downgraded to LOW/Accepted (SEC-M2).

**Scope:** the surface added since v0.20.0 — the own RFC 8484 DoH h2 handler + the public→loopback relay's abuse enforcement (`src/dns/server.rs`), the local-CA leaf signer + `GET /api/tls/ca` (`src/api/mod.rs`), the DDoS abuse engine (verified-source anti-spoof gate, tarpit, kernel ban via the `bans_active`-gated check in `ebpf/dns_xdp.c` + `xdp/loader.rs` + `main.rs`, `src/alerts.rs`), actor-attributed audit (`src/audit.rs` + the auth-middleware `admin_action`), the alert-rule editor API (`PUT /api/alerts/rules`), and the `/metrics` additions (#208). The kernel-UDP/AF_XDP packet code is unchanged except the one gated ban lookup (bench-validated, below).

| ID | Severity | Status | Finding |
|---|---|---|---|
| SEC-M1 | HIGH | **Fixed** | The alert-rule writer emitted `name` / `metric` / `action` / `notify-url` into `runbound.conf` with raw `format!` instead of `escape_str` (which neutralises quotes and newlines). `PUT /api/alerts/rules` validated the rule *name* for control chars but **not** `notify-url`. A malicious `notify-url` (admin-token-reachable) with an embedded newline could break out of the quoted value and inject a separate config directive on the next regeneration — the SEC-J1 class (config-directive injection → arbitrary directive such as a command-running hook → RCE as the service user). Fixed: `escape_str` on all four alert string fields in `config/writer.rs`, plus a control-char/length (≤2048) check on `notify-url` in `put_alert_rules`. |
| SEC-M2 | LOW | **Accepted** | API error bodies (e.g. `tls_ca_handler`, `put_alert_rules`) return the internal error text via `e.to_string()`, aiding stack fingerprinting. Reachable only by an authenticated caller (admin / API key) and consistent with the pre-existing handler pattern across the codebase; an authenticated admin is already privileged. Accepted (defence-in-depth only). |
| SEC-M3 | MEDIUM→**Disputed** | Disputed (false positive) | Gemini claimed `POST /api/tls/self-signed` "removed hostname length/character validation", enabling a resource-exhaustion DoS. Refuted: the handler validates `host` as 1..253 chars with no control characters (`api/mod.rs`); the validation was never removed — only the certificate *signing* path changed (bare self-signed → local-CA-signed). No DoS vector. |

**Datapath note:** the only `ebpf/`/`src/dns/xdp/` change is the `bans_active`-gated per-packet ban lookup (skipped via a single BPF array read when no IP is banned). A/B X710 NIC-truth bench — gate binary vs the v0.20.0 pre-gate baseline, two rounds with order reversed — measured **10.09–10.11 M qps served** both ways (Δ ±0.12 %, within noise), so the fast path is not slowed; the program passed the kernel verifier at load.

**Live pentest (throwaway instance, two-AI):** a Claude attack battery + a Gemini 2.5 Pro vector list were run against a disposable build of the new surface. **No exploitable finding.** SEC-M1 is verified closed live (a `notify-url` with an embedded newline → `400 INVALID_NOTIFY_URL`; a rule name with a `"` → 200 but written escaped as `name: "x\" …"`, no injected directive, no file created). All 12 Gemini-proposed vectors were refuted at the code: SSRF via `notify-url` is blocked by the pre-existing `is_safe_webhook_url()` (private/loopback/scheme reject); `X-Forwarded-For` is never trusted (VUL-04); victim-ban-via-spoof is infeasible (escalation needs a verified source — a connection or a per-boot-HMAC DNS cookie that cannot be forged); the `metric` field is whitelisted to `client-qps`; rule sets are capped at 64; `/metrics` carries no client/secret labels and is a cheap render; the audit view HTML-escapes actor/event/path. Remaining items are admin-gated, mitigated LOWs (self-signed-cert CPU, h2 stream flood bounded by the per-IP connection cap, ban/IP-rotation latency ≈ the 1 s detector tick) — none reachable unauthenticated and none a new vulnerability. Endpoint auth verified: `/metrics`, `/audit/tail`, `/alerts/rules` all return 401 without a token; DoH rejects oversized bodies (400) and bad methods (405).

**Functional cross-check (two-AI, 100 %):** a full sweep (every API GET 200; DNS over UDP/TCP/DoT/DoH-GET/DoQ resolves; relay + SSE; audit `admin_action`; metrics) plus a Gemini regression-risk list. One real gap was found and **fixed**: the abuse engine exempted loopback in the handler but **not in the TCP relay**, so a local-process TCP flood could self-ban/tarpit `127.0.0.1` — now the relay also skips loopback (verified live: 10 loopback TCP queries past the threshold, 0 tarpitted). All other Gemini risks were refuted: alert-rule hot-apply does **not** reload the TLS listeners; DoH and UDP share the transport-agnostic cache key (same `RunboundHandler`); the audit write is async (no query-path latency) and only fires from the API middleware; the XDP fast path is unchanged for non-banned traffic (bench above). No regression in the existing UDP/TCP/DoT/DoH/DoQ, recursion, hot-reload, relay or XDP paths. (`/health` is intentionally at the root, not under `/api`, for unauthenticated LB probes.)
...
| SEC-L7 | LOW | **Fixed** | CNAME chains in a signed local zone are now signed end-to-end (`ZoneSigner::sign_chain` signs each RRset of the chain when the client set DO); wildcards remain a follow-up. Previously `sign_answer` did not follow and sign CNAME chains — a DNSSEC-correctness gap for those record shapes, not a forgery (it also removes any type/algorithm-confusion surface in the RRSIG, since the signed RRset is type-homogeneous). Tracked; local-zone CNAME/wildcard is uncommon. |
...
| SEC-L10 | INFO | **Fixed** | The `/api/tls/*` mutating handlers now carry an internal `caller.admin` check (defence-in-depth) in addition to the deny-by-default middleware allow-list and the slave-guard. Verified: a `read`-role key gets 403, admin gets 200. |
...
| SEC-L12 | LOW | **Accepted** | The per-IP DNS rate-limiter is a coarse flood mitigator (a per-thread 10 ms-allow / 100 ms-deny decision cache for zero per-packet cost), not a precise QPS cap: a sustained flood is reduced (~58% dropped at 169k/s in functional testing) but the served rate is not pinned to the configured value. Accepted as the existing performance-vs-precision trade-off on the byte-identical datapath; a precise token cap would add per-packet cost on the hot path. |
| OPEN-L1 | LOW | **Open** | Enhancement: cache the signed RRSIG and the NSEC3 chain per (zone, owner-set) with a TTL so a DO=1 flood of random names in a signed zone cannot force repeated signing. SEC-L1 removed the per-RR key reconstruction (the dominant cost). A global concurrency cap was evaluated and rejected (signing is synchronous per worker thread, already bounded by thread count, so a cap does not shed a CPU-saturation flood); the coherent per-answer cache, invalidated on zone change, is the correct remaining work and is deferred to avoid stale-denial risk before the release. |
...
**Date:** 2026-06-14  
**Scope:** the new attack surface introduced since Cycle J — the **anycast** feature only (`src/anycast.rs`, the `anycast:` arms of `config/parser.rs` and `config/writer.rs`, the `/api/system` + relay `/system` exposure in `api/mod.rs`/`sync.rs`, and the `XDP_MODE` status global). The DNS packet hot path was **not** re-reviewed: the only change under `src/dns/` since v0.18.1 is the additive read-only `XDP_MODE` global + its two accessors in `xdp/socket.rs` (status only, never on the packet path) — the socket/ring/recv/tx code is byte-identical (`git diff v0.18.1..HEAD -- src/dns/` shows only those additions). Cycles H/I/J cover the data path.  
