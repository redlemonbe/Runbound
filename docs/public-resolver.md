...
Runbound already serves **DoT (853)**, **DoH (443)** and **DoQ (853/UDP,
RFC 9250)**, and forwards upstream over DoT. DoQ is a first-class QUIC listener
built directly on `quinn` (`src/dns/doq.rs`, `spawn_doq` bound on UDP by
`src/dns/server.rs`), on parity with DoT/DoH. This guide turns those transports into a real public,
...
| 853 | UDP | DoQ (RFC 9250, QUIC) — served on UDP via `quinn` |
...
DoT/DoH/DoQ auto-activate once `tls-service-pem`/`-key` are set — all three
share the same cert (see §1). Certificate changes are **applied live**
...
the in-house wire codec), one record per transport:
...
`quic-port` (defaults 853 / 443 / 853;
`quic-port` configures the live DoQ listener, see §1).
...
`:53` surface. (DoQ is QUIC/UDP-based — see §1; QUIC address-validation caps amplification, and
like DoT/DoH it is not a spoofed-amplification vector because the QUIC handshake
