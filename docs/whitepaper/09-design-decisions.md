...
> **Status: current (0.9.3, last full sync pass: 2026-07-09)** — condensed decision table.
...
| **Wire-native default handler** | A dedicated per-query codec avoids the generic-codec overhead of a `ServerFuture`-style path; the in-house wire codec (`src/dns/wire/`, `serve_wire`) serves forward/full-recursion/local/AXFR/TSIG/DDNS/DNSSEC-signing on every path, and the request path is wire-native throughout the runtime | A full DNS message/codec to own and keep RFC-correct (guarded by differential oracles vs `hickory-proto` + `delv`); `hickory-proto` is a `[dev-dependencies]` entry for those oracle tests |
| **Sovereign recursion, always compiled in** | Full iterative recursion (`src/dns/recursor_wire.rs`) and DNSSEC validation (`src/dns/dnssec_*.rs`) are entirely in-house and always compiled in (no Cargo feature gates them) — but OFF by runtime default: `resolution: forward` and `dnssec-validation: no` are the defaults; full-recursion and DNSSEC validation are opt-in via config (`resolution: full-recursion`, `dnssec-validation: yes`), not a build flag | The in-house recursor + validator must be kept RFC-correct without falling back to a library implementation |
