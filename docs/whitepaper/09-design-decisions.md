...
> **Status: current (v0.22.4)** — condensed decision table.
...
| **De-hickory: wire-native default handler (v0.22)** | The hickory `ServerFuture` per-query `tokio::spawn` + generic codec was 1.78× Unbound's instructions; the in-house wire codec (`src/dns/wire/`, `serve_wire`) serves forward/local/AXFR/TSIG/DDNS/DNSSEC-signing without it, and removing the hickory request handler from the default binary shrinks the network-facing attack surface | A full DNS message/codec to own and keep RFC-correct (guarded by differential oracles vs hickory + `delv`); `hickory-proto` still backs the data model + XDP builders |
| **Sovereign recursion behind `recursor` feature** | Full iterative recursion still uses `hickory-resolver`, but most deployments forward — so it is an optional Cargo feature, kept off the default build | A second serving path, only built/tested when the feature is on |
