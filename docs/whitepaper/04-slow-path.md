...
> They are, however, **off by runtime default**: `resolution: forward` and
> `dnssec-validation: no` are the defaults (`UnboundConfig::defaults()`,
> `src/config/parser.rs`); full-recursion and DNSSEC validation are opt-in via config
> (`resolution: full-recursion`, `dnssec-validation: yes`), not a build flag.
> `hickory-proto` is only a `[dev-dependencies]` entry for the differential oracle tests.
...
AXFR, DDNS, DNSSEC signing) are handled by `serve_wire`. The **fallback**
pipeline:
...
`xdp: no` is **not** one async task per query. The receive side is a tight kernel loop
...
- **One `SO_REUSEPORT` UDP socket per physical core** (minus one reserved for the rest of
  ...
  uneven for a few flows, so an `SO_ATTACH_REUSEPORT_CBPF` program returning
...
Forwarding is wire-native — the in-house forward pool (`src/dns/forward.rs`):
...
modes (#183). The `serve_wire` **fallback** still infers a hit from latency (a lookup under
...
and **~7.3 M** on **ixgbe/X520** (a different datapath, **not
...
## 4.6 Feature parity on the wire path
