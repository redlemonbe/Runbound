...
Runbound includes an AF_XDP fast path, enabled by default (the `xdp`
feature is part of the default feature set).

## License

Runbound is licensed under AGPL-3.0 and the AF/XDP fast path is **included and
enabled by default** — no activation step is required. A commercial license is
available for organizations that need to deploy without AGPL obligations (it
covers the whole product, fast path included): see
...
> The slow path that XDP falls through to is wire-native (`serve_wire`). Note that
> **XDP mode is what requires the extra capabilities**
> (`CAP_NET_RAW`/`CAP_NET_ADMIN`/`CAP_BPF`/`CAP_PERFMON`) — see
> [hardening.md](hardening.md).

## SIMD acceleration in the XDP worker
...
Measured on an X520 10 GbE link (same `a.bench.test A` query, dnsmark 1.0,
