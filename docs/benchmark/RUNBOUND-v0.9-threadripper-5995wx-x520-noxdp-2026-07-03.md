# Runbound v0.9 — threadripper-5995wx — X520/82599ES (ixgbe) — `xdp:no` — 2026-07-03
...
- **Receiver (Runbound v0.9):** dragonrage — Threadripper PRO 5995WX, 128 cores, 125 GiB RAM, kernel `7.0.6-2-pve`, governor `performance`. Runbound v0.9, `xdp: no` (kernel slow path), `/root/rb-bench-noxdp.conf`: real `forward-zone` 1.1.1.1/8.8.8.8/9.9.9.9, `cache-size 131072`, `rate-limit 0`, interface 0.0.0.0, no `local-data`; started via systemd. `:53` sole owner verified.
