# Runbound v0.9 — threadripper-5995wx — dual-link X710+X520 — `xdp:yes` — 2026-07-03
...
Measure whether the AF_XDP fast path scales across two links simultaneously (one unified dnsmark multi-NIC DSD).
...
- **Generator (dnsmark):** dragonsage (dual Xeon E5-2690 v2), **dnsmark 1.0** + **dnsperf 2.14.0**. AF_XDP multi-NIC generator: `dnsmark -s 10.71.10.1 -s 10.51.10.1 -d /root/queries-A.txt --xdp --ramp` (single unified DSD across both links) and `… --xdp -Q 0 --max-outstanding 0 -l 20`.
