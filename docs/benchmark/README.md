...
counter before it goes into a report. Use dnsmark ≥ v2.5.0 (earlier versions
under-counted the round-trip by up to ~15× in firehose mode — the reason rule 1
exists); prefer the latest signed release.
...
- Record the dnsmark version. Multi-NIC ramps require ≥ v2.6.1 (single unified DSD
  across links: one aggregate knee + per-NIC breakdown; earlier versions ran one
  independent ramp per NIC, which cross-coupled through the shared server CPU and did
  not converge).
