# Runbound v0.23.13 — threadripper-5995wx — X710 (i40e) — `xdp:yes` — 2026-07-03
...
Measure the Runbound AF_XDP fast path on a single X710 link under the revised methodology (dnsmark v2.7.7), back-to-back with the other same-day runs.
...
- **Generator (dnsmark):** dragonsage (dual Xeon E5-2690 v2), **dnsmark 2.7.7** + **dnsperf 2.14.0**. AF_XDP generator (`--xdp`), egress NIC X710 i40e. Exact: `dnsmark -s 10.71.10.1 -d /root/queries-A.txt --xdp --ramp` and `… --xdp -Q 0 --max-outstanding 0 -l 20`.
