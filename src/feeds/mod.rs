...
fn feeds_config_path() -> std::path::PathBuf {
    crate::runtime::base_dir().join("feeds.json")
}
fn feed_cache_dir() -> std::path::PathBuf {
    crate::runtime::base_dir().join("feed_cache")
}
...
pub fn load_feeds() -> Result<FeedsConfig, AppError> {
    ...
    let content = fs::read_to_string(&path)
        .map_err(|e| AppError::Internal(format!("Failed to read feeds config: {}", e)))?;
    ...
    serde_json::from_str(&content)
        .map_err(|e| AppError::Internal(format!("Failed to parse feeds config JSON: {}", e)))
}

pub fn save_feeds(config: &FeedsConfig) -> Result<(), AppError> {
    let path = feeds_config_path();
    let content = serde_json::to_string_pretty(config)
        .map_err(|e| AppError::Internal(format!("Failed to serialize feeds config: {}", e)))?;
    let tmp = path.with_extension("json.tmp");
    {
        let mut f = fs::File::create(&tmp)
            .map_err(|e| AppError::Internal(format!("Failed to create temp feeds file: {}", e)))?;
        f.write_all(content.as_bytes())
            .map_err(|e| AppError::Internal(format!("Failed to write temp feeds file: {}", e)))?;
        ...
        f.sync_all()
            .map_err(|e| AppError::Internal(format!("Failed to fsync feeds file: {}", e)))?;
    }
    fs::rename(&tmp, &path)
        .map_err(|e| AppError::Internal(format!("Atomic rename of feeds file failed: {}", e)))?;
    #[cfg(unix)]
    {
...
pub fn load_feed_domains(feed_id: &str) -> Vec<String> {
    ...
    if let Err(e) = verify_mac(&path, content.as_bytes(), store_key().as_deref()) {
        warn!(
            "Feed domain cache {} HMAC mismatch ({e}) — cache discarded.",
            path.display()
        );
        return Vec::new();
    }
...
fn save_feed_domains(feed_id: &str, domains: &[String]) -> Result<(), AppError> {
    ...
    let path = cache_path(feed_id);
    let tmp = path.with_extension("json.tmp");
    let content = serde_json::to_string(domains)
        .map_err(|e| AppError::Internal(format!("Failed to serialize feed domains: {}", e)))?;
    {
        let mut f = fs::File::create(&tmp).map_err(|e| {
            AppError::Internal(format!("Failed to create temp domain cache: {}", e))
        })?;
        f.write_all(content.as_bytes())
            .map_err(|e| AppError::Internal(format!("Failed to write temp domain cache: {}", e)))?;
        f.sync_all()
            .map_err(|e| AppError::Internal(format!("Failed to fsync domain cache: {}", e)))?;
    }
    fs::rename(&tmp, &path)
        .map_err(|e| AppError::Internal(format!("Atomic rename of domain cache failed: {}", e)))?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let _ = fs::set_permissions(&path, fs::Permissions::from_mode(0o640));
...
// The single SSRF address filter shared by feeds AND webhooks (SEC-2026-07-B/C).
// Defined as the exact logical inverse of `recursor_wire::is_public_ip` so the two
...
pub async fn add_feed(
    ...
    if name.is_empty() || name.len() > 128 {
        return Err(AppError::BadRequest(
            "Feed name must be 1-128 characters".into(),
        ));
    }
...
fn is_valid_domain(s: &str) -> bool {
    ...
    for label in s.split('.') {
        if label.is_empty() {
            return false;
        } // consecutive or leading/trailing dots
        if label.len() > 63 {
            return false;
...
pub async fn update_feed(feed: &mut Feed, client: &reqwest::Client) -> Result<usize, AppError> {
    ...
    if response
        .content_length()
        .map(|n| n as usize > MAX_FEED_BYTES)
        .unwrap_or(false)
    {
        return Err(AppError::Internal(format!(
...
mod tests {
    ...
    fn is_private_ip_ipv4_compatible_loopback() {
        ...
        assert!(
            is_private_ip(&ip),
            "::127.0.0.1 must be treated as private (IPv4-compatible loopback)"
        );
    }
    ...
    fn is_private_ip_ipv4_compatible_rfc1918() {
        ...
        assert!(
            is_private_ip(&ip),
            "::10.0.0.1 must be treated as private (IPv4-compatible RFC1918)"
        );
    }
    ...
    fn is_private_ip_ipv4_mapped_loopback() {
        ...
        assert!(
            is_private_ip(&ip),
            "::ffff:127.0.0.1 must be treated as private (IPv4-mapped loopback)"
        );
    }
    ...
    fn is_private_ip_public_v4_allowed() {
        ...
        assert!(
            !is_private_ip(&ip),
            "8.8.8.8 must not be treated as private"
        );
    }

    #[test]
    fn is_private_ip_public_v6_allowed() {
        // A genuinely routable public v6 (Cloudflare) — 2001:db8::/32 is documentation
        // and is (correctly) blocked, so it is not a valid "public" fixture.
        let ip: std::net::IpAddr = "2606:4700:4700::1111".parse().unwrap();
        assert!(!is_private_ip(&ip), "a public v6 must not be treated as private");
    }

    #[test]
    fn is_private_ip_blocks_special_ranges() {
        // SEC-2026-07-B/C re-audit: unified with is_public_ip — special ranges blocked.
        for s in ["255.255.255.255", "192.0.2.10", "198.18.0.1", "240.0.0.1", "::ffff:10.0.0.1"] {
            assert!(is_private_ip(&s.parse().unwrap()), "{s} must be blocked");
        }
        // IPv4-mapped PUBLIC address must NOT be over-blocked.
        assert!(!is_private_ip(&"::ffff:8.8.8.8".parse().unwrap()), "mapped public must pass");
    }

    #[test]
    fn is_private_ip_blocks_cgnat() {
        // SEC-2026-07-C: RFC 6598 CGNAT 100.64.0.0/10 must be blocked (SSRF).
        for s in ["100.64.0.1", "100.100.50.50", "100.127.255.255"] {
            assert!(is_private_ip(&s.parse().unwrap()), "{s} (CGNAT) must be blocked");
        }
        // Just outside the /10 stays public.
        for s in ["100.63.255.255", "100.128.0.1"] {
            assert!(!is_private_ip(&s.parse().unwrap()), "{s} is outside CGNAT /10");
        }
    }
}
