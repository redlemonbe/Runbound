...
pub fn verify_mac(
    ...
    match (key, mac_exists) {
        ...
        (Some(_), false) => {
            // SEC-2026-07-D: key configured but no sidecar. Fail CLOSED — an attacker with
            // filesystem write could delete the .mac to silently bypass integrity. A one-shot
            // migration (RUNBOUND_STORE_MAC_MIGRATE=1) lets pre-existing unprotected stores
            // load once so their sidecars can be (re)written.
            if std::env::var("RUNBOUND_STORE_MAC_MIGRATE").ok().as_deref() == Some("1") {
                // Self-heal: write the missing sidecar now from the current on-disk content
                // so migration is genuinely one-shot (next boot is protected, the flag can be
                // unset). Re-audit 2026-07-04 (SEC-2026-07-D): the previous version only
                // "loaded once" and relied on a later save to write the sidecar.
                if let Some(k) = key {
                    match write_mac(path, content, Some(k)) {
                        Ok(()) => warn!(
                            path = %path.display(),
                            "RUNBOUND_STORE_KEY set but no .mac sidecar — MIGRATION: wrote the \
                             sidecar from current content. Unset RUNBOUND_STORE_MAC_MIGRATE now."
                        ),
                        Err(e) => warn!(
                            path = %path.display(), err = %e,
                            "MIGRATION: loading once but failed to write .mac sidecar (check FS perms)."
                        ),
                    }
                }
                Ok(())
                ...
                tracing::error!(
                    path = %path.display(),
                    "RUNBOUND_STORE_KEY is set but no .mac sidecar found — refusing to load. \
                     Set RUNBOUND_STORE_MAC_MIGRATE=1 for a one-shot migration if this store \
                     predates integrity protection."
                );
                Err(format!(
                    "missing .mac sidecar for {} (integrity required)",
                    path.display()
                ))
            }
        }
