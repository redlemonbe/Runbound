...
pub fn hmac_verify_with_ts(
    ...
    // SEC-J5: only the body-covering signature is accepted; the legacy header-only
    // fallback (which left the request body unauthenticated) is rejected. The relay
    // channel is TLS-pinned; this is defence in depth.
...
impl SyncJournal {
    ...
    pub fn register_node(
        ...
        // SEC-2026-07-G: TOFU — once a node's cert_fingerprint is pinned, a later
        // registration must not silently change it (that would redirect relay pushes
        // to an attacker-controlled host). Only the real slave holds the matching cert,
        // so a fingerprint match still lets relay_host/version update. Re-pinning
        // requires removing the node first (admin action).
        if let Some(existing) = nodes.get_mut(&node_id) {
            if existing
                .cert_fingerprint
                .as_deref()
                .is_some_and(|p| !p.is_empty() && p != cert_fingerprint)
            {
                tracing::warn!(
                    node_id = %node_id,
                    "register: cert_fingerprint change rejected for an already-pinned node — \
                     remove the node first to re-pin. Refreshing liveness only."
                );
                existing.last_seen_at = now;
                existing.last_seen_secs = 0;
...
async fn handle_relay_request(
    ...
    match (method.as_str(), op) {
        ...
        ("PUT", "resolution") => {
            ...
            {
                // The in-house validating resolver is stateless; applying a mode
                // is just flipping the hot-path atomic.
