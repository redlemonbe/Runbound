...
fn answer_from_cache(
    ...
    if let Some(entry) = cache_snap.get(&key) {
        ...
        if entry.expires_at > now && entry.wire_payload.len() >= 2 {
            ...
            tx_dns[0] = qid[0];
            tx_dns[1] = qid[1];
            // Echo the client's RD bit (flags byte [2], mask 0x01) so the cache fast
            // path mirrors the slow path — a name resolves identically either way.
            tx_dns[2] = (tx_dns[2] & 0xFE) | (query_bytes[2] & 0x01);
            ...
            // an earlier regression came from shared atomics, which these avoid). This makes
            ...
            // Per-hit observability, contention-free AND high-cardinality-safe:
            ...
            // every hit crushed XDP throughput ~26× at high domain cardinality.
