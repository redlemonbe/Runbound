...
// Differential tests cross into `dns::local` / `dns::wire_bridge`, which the
// `fuzz` library build (lib.rs) deliberately does not expose. Excluding them
// under `fuzz` keeps `cargo test --all-features` / `clippy --all-targets
// --all-features` compiling without dragging those modules into the fuzz crate.
