...
pub struct WireQuery<'a> {
    ...
    pub qclass: u16,
    /// Recursion-Desired bit from the query header. Echoed back on the fast-path
    /// response so a name resolves identically to the slow path (which mirrors RD).
    pub rd: bool,
...
pub fn parse_query(buf: &[u8]) -> Option<WireQuery<'_>> {
    ...
    Some(WireQuery {
        ...
        qtype,
        qclass,
        rd: flags & 0x0100 != 0, // RD is bit 8 of the 16-bit flags field
        edns,
    })
}
...
pub fn build_answer_a_aaaa(
    ...
    pos = put_u16(out, pos, (FLAGS_AA_NOERROR & 0xFEFF) | (u16::from(wq.rd) << 8)); // QR AA RA NOERROR, RD echoed
...
pub fn build_answer_a_aaaa_wire(
    ...
    // Header (12 bytes). RD (0x0100) is echoed from the query, not hard-set — the
    // fast path must mirror the slow path's RD so the answer is path-independent.
    let flags_aa: u16 = 0x8480 | (u16::from(wq.rd) << 8); // QR=1 AA=1 RA=1 RCODE=0, RD=query
    let arcount: u16  = if edns.is_some() { 1 } else { 0 };
    let mut p = 0;
...
pub fn build_nxdomain(wq: &WireQuery<'_>, out: &mut [u8], edns: Option<&EdnsInfo>) -> Option<usize> {
    ...
    write_header_and_question(out, wq.id, flags, 0, wq.qname_wire, wq.qtype, wq.qclass, edns)
}
...
pub fn build_nodata(wq: &WireQuery<'_>, out: &mut [u8], edns: Option<&EdnsInfo>) -> Option<usize> {
    // FLAGS_AA_NOERROR = 0x8580 (QR=1 AA=1 RA=1 RCODE=0); RD echoed from the query.
    let flags = (FLAGS_AA_NOERROR & 0xFEFF) | (u16::from(wq.rd) << 8);
    write_header_and_question(out, wq.id, flags, 0, wq.qname_wire, wq.qtype, wq.qclass, edns)
}
...
pub fn build_refused(wq: &WireQuery<'_>, out: &mut [u8], edns: Option<&EdnsInfo>) -> Option<usize> {
    let flags = (FLAGS_REFUSED & 0xFEFF) | (u16::from(wq.rd) << 8); // RD echoed from query
    write_header_and_question(out, wq.id, flags, 0, wq.qname_wire, wq.qtype, wq.qclass, edns)
}
...
mod tests {
    ...
    fn build_nxdomain_wire_correct() {
        ...
        let wq = WireQuery { id: 0xABCD, qname_wire: qname, qtype: 1, qclass: 1, rd: true, edns: None };
        let mut out = [0u8; 512];
    ...
    fn build_nodata_wire_correct() {
        ...
        let wq = WireQuery { id: 0x1234, qname_wire: qname, qtype: 28, qclass: 1, rd: true, edns: None };
        let mut out = [0u8; 512];
    ...
    fn build_refused_wire_correct() {
        ...
        let wq = WireQuery { id: 0xFFFF, qname_wire: qname, qtype: 1, qclass: 1, rd: true, edns: None };
        let mut out = [0u8; 512];
    ...
    fn build_nxdomain_buf_too_small() {
        ...
        let wq = WireQuery { id: 0x0001, qname_wire: qname, qtype: 1, qclass: 1, rd: true, edns: None };
    ...
    fn build_negative_echo_qtype_aaaa() {
        ...
        let wq = WireQuery { id: 0x0042, qname_wire: qname, qtype: 28, qclass: 1, rd: true, edns: None };
        let mut out = [0u8; 512];
    ...
    fn build_nxdomain_with_edns_opt_echo() {
        ...
        let wq = WireQuery { id: 0x1111, qname_wire: qname, qtype: 1, qclass: 1, rd: true, edns: None };
    ...
    fn build_nxdomain_flags_unchanged_with_edns() {
        ...
        let wq   = WireQuery { id: 0xBEEF, qname_wire: qname, qtype: 1, qclass: 1, rd: true, edns: None };
