...
pub struct LocalZoneSet {
    ...
    /// The full record set as our own `wire::Record`,
    ...
    /// Zone actions keyed by lowercased wire QNAME, the
...
impl LocalZoneSet {
    pub fn from_config(zones: &[LocalZone], data: &[LocalData]) -> Self {
        ...
        // ── Build the wire::Record store from the same lines ─────────────────
    ...
    for (key_raw, entry) in &zones.wire_records.map {
        for qtype in [1u16, 28u16] {
            ...
            let wq = WireQuery {
                ...
                qtype,
                qclass: 1,
                // Cache-entry template: RD is patched per query in answer_from_cache.
                rd: true,
                edns: None,
            };
            let mut buf = [0u8; 512];
