...
impl UserRegistry {
    ...
    pub fn by_api_key(&self, key: &str) -> Option<Arc<UserAccount>> {
        // SEC-2026-07-E: constant-time — scan all keys, compare each without early-exit,
        // so lookup latency does not depend on the candidate key's content (parity with
        // the master key's constant_time_eq). User keys are uniform-length CSPRNG tokens.
        use subtle::ConstantTimeEq;
        let mut found: Option<Arc<UserAccount>> = None;
        for entry in self.by_key.iter() {
            if entry.key().as_bytes().ct_eq(key.as_bytes()).into() {
